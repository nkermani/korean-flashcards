# gui/ui/layout.py
# This module sets up the main UI layout for the flashcard application using Tkinter.
...
def setup_layout(
    ...
    create_label(
        root, textvariable=word_var, row=0, column=1, sticky="w", font=bold_font
    )
    create_label(root, "Definition:", 1, 0, sticky="w")
    create_label(
        root, textvariable=def_var, row=1, column=1, sticky="w", wraplength=300
    )
    create_label(root, "Example:", 2, 0, sticky="w")
    create_label(
        root, textvariable=example_var, row=2, column=1, sticky="w", wraplength=300
    )
    create_label(root, "Synonyms:", 3, 0, sticky="w")
    create_label(
        root, textvariable=synonyms_var, row=3, column=1, sticky="w", wraplength=300
    )
    create_label(root, "Antonyms:", 4, 0, sticky="w")
    create_label(
        root, textvariable=antonyms_var, row=4, column=1, sticky="w", wraplength=300
    )
    ...
    if saved_files:
        ...
        create_button(
            ...
            command=(
                lambda: (
                    load_file_cb(listbox.get(tk.ACTIVE))
                    if (load_file_cb and listbox)
                    else None
                )
            ),
            columnspan=2,

...
# start.sh — start FastAPI first, wait until it's ready, then start the Tkinter GUI.
# Writes logs to ./logs, forwards signals to children, and stops the other when one exits.
...
python app/main.py > "$FASTAPI_LOG" 2>&1 &
FASTAPI_PID=$!
...
for i in $(seq 1 $START_TIMEOUT); do
	...
	# if the process exited early, break and handle below
